...
if(!function_exists('jevelin_shortcode_accordion_css')) :
	function jevelin_shortcode_accordion_css ($data) {
		$atts = jevelin_shortcode_options($data,'accordion');
		...
		ob_start(); ?>
			...
			#accordion-<?php echo esc_attr( $atts['id'] ); ?>.sh-accordion .panel-title a i {
				...
				<?php /* Active background color */
					$accent_color = ( esc_attr($atts['expanded_background_color']) ) ? $atts['expanded_background_color'] : jevelin_option('accent_color');
					if( $accent_color ) : ?>
			...
			<?php if( $atts['icon_color'] ) : ?>
				#accordion-<?php echo esc_attr( $atts['id'] ); ?> .sh-accordion-icon i.open-icon {
					color: <?php echo esc_attr( $atts['icon_color'] ); ?>!important;
				}
			<?php endif; ?>


			#accordion-<?php echo esc_attr( $atts['id'] ); ?>.sh-accordion-style5 .nav-tabs li.active:after {
				<?php if( $accent_color ) : ?>
					background-color: <?php echo esc_attr($accent_color); ?>;
				<?php endif; ?>
			}
